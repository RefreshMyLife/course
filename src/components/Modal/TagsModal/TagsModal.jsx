...
import { addTags, deleteTags, removeTags, toggleTagsModal } from '../../../features';
...
const TagsModal = ({ type, addedTags, handleTags }) => {
  ...
  const test = useSelector((state) => state.notesList.mainNotes);
  const [inputText, setInputText] = useState('');
  console.log(test);
  const postTag = async (tagName, NoteId) => {
    await TagService.addTag(tagName, NoteId);
  };
  //add tags
    ...
    <FixedContainer>
      <Box>
        <div className="editTags__header">
          <div className="editTags__title">{type === 'add' ? 'Добавить' : 'Редактировать'} тег</div>
          <DeleteBox
